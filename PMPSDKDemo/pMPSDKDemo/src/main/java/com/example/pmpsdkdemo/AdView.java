			...
			mRequestThread = new Thread(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						Response response = new AdImageRequest(mRequestParams).sendRequest();
						if(response != null){
	...
	private Handler mHanlder = new Handler(){
		public void handleMessage(android.os.Message msg) {
			...
			case MSG_SHOW_AD:{
				...
				if(response != null && AdSDKManagerCompat.isAviableResponse(response)
						&& isWindowAdded){
					if(mFloatView == null){
						mFloatView = new FloatView(AdView.this.getContext(), response);
						AdView.this.addView(mFloatView, new RelativeLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT));
					}
